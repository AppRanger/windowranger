...
| CR-005 | Low / Debug performance | Verbose Debug diagnostics serialize and append synchronously while holding the logger lock. This preserves ordered JSON Lines and does not exist in Release, but a slow filesystem could add Debug-only latency during a very noisy live session. | `DiagnosticLogger.log` and `RotatingFileDiagnosticSink.append`. | **Controlled measurement complete; live validation remains.** On 10 August 2026, 100 correlated 256-byte-payload records took 4.2 ms with the memory sink and 311.5 ms with 2 ms latency injected per ordered append (3.11 ms/record), preserving sequence 1...100. The result confirms linear blocking under deliberately slow writes but does not prove a user-visible problem at real Debug event rates. Keep the ordered implementation until the signed-app slow-storage session in WR-005 supplies that evidence. |
